...
func decodeSCTPChunk(data []byte) (SCTPChunk, error) {
	...
	return SCTPChunk{
		...
		Length:       length,
		ActualLength: actual,
		BaseLayer:    BaseLayer{data[:actual], data[actual : len(data)]},
	}, nil
}
...
func decodeSCTPData(data []byte, p gopacket.PacketBuilder) error {
	...
	sc := &SCTPData{
		...
		Payload:         data[16:chunk.Length],
	}
	if len(sc.Payload) + 16 < int(chunk.Length) {
		return fmt.Errorf("SCTP data short. chunk length(%v) vs. payload(%v)\n", chunk.Length, sc.Payload)
	}
	p.AddLayer(sc)
	return p.NextDecoder(gopacket.DecodeFunc(decodeWithSCTPChunkTypePrefix))
}
